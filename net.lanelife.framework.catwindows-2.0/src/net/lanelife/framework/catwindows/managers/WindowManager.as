		...
		public function bringToFront(window:Window):Boolean
		{
			if (window!=front)
			{
				WindowEventDispatcher.getInstance().dispatchEvent(
					new WindowEvent(WindowEvent.WINDOW_DEPTH_CHANGE+SystemVariables.currentDesktopScreenIndex, window)
				);
				if (front)
				{
				...
				if(front is DesktopWindow) {
					var desktopWindow:DesktopWindow = DesktopWindow(front);
					desktopWindow.taskbarButton.flashes = false;
					desktopWindow.taskbarButton.active = true;
				}
